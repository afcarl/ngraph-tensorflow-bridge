...
#flags.DEFINE_bool("use_fast_weights",1,"entr fwlst")
flags.use_fast_weights=1
FLAGS = flags.FLAGS
...
class PTBModel(object):
  ...
  def __init__(self, is_training, config):
    ...
    with tf.device('/device:NGRAPH:0'):
        ...
        cell =lstm_cell# tf.contrib.rnn.MultiRNNCell([lstm_cell] * config.num_layers)

        self._initial_state = cell.zero_state(batch_size, tf.float32)

        #with tf.device("/cpu:0"):
        embedding = tf.get_variable("embedding", [vocab_size, size])
        inputs = tf.split(tf.nn.embedding_lookup(embedding, self._input_data),
               num_steps,1)
        ...
        with tf.variable_scope("RNN",reuse=tf.AUTO_REUSE):
          #with tf.variable_scope("RNN",reuse=tf.AUTO_REUSE):
          outputs, states = tf.nn.static_rnn(cell, inputs, initial_state=state)
          '''
...
def run_epoch(session, m, data, eval_op, verbose=False):
  ...
  for step, (x, y) in enumerate(reader.ptb_iterator(data,m.batch_size,m.num_steps)):#:, batch_size,
                                                    #num_steps)):
    #import ipdb;ipdb.set_trace()
...
def main(unused_args):
  ...
  with tf.Graph().as_default(), tf.Session(config=config_proto) as session:
    ...
    for i in range(config.max_max_epoch):
      lr_decay = config.lr_decay * max(i+1 - 13, 1.0)
